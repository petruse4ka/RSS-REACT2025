...
export default function CountryItem({ country, mainTableFields, additionalTableFields }: Props) {
  ...
  return (
    <tr
      className={`hover:bg-scooter-400 dark:hover:bg-shamrock-400 transition-colors duration-150`}
    >
      <td className="px-6 py-4 text-sm whitespace-nowrap text-zinc-700 dark:text-zinc-50">
        {country.name}
      </td>
      <td className="px-6 py-4 text-sm whitespace-nowrap text-zinc-700 dark:text-zinc-50">
        {country.iso_code}
      ...
      {mainTableFields.map((field) => {
        ...
        return (
          <td
            key={field.key}
            className="px-6 py-4 text-sm whitespace-nowrap text-zinc-700 dark:text-zinc-50"
          >
